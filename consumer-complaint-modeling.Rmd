...
rf_tunedd_overallfit <-tune::last_fit(rf_tunedd_wflow, split = rf_modeling_data_split)
...
set.seed(456)
final_fit_resample <- tune::fit_resamples(final_wflow, 
                                          rf_vfolds, 
                                          control = control)
collect_metrics(final_fit_resample)

final_pred_test_values <- collect_predictions(final_fit_resample)


final_pred_test_values |>
  pivot_longer(c(.pred_class, product_value), names_to = "pred_or_orig", values_to = "products") |> 
  group_by(products, pred_or_orig) |> 
  summarise(count = n()) |> 
  group_by(pred_or_orig) |> 
