...
set.seed(456)
resample_final_fit <- tune::fit_resamples(final_wflow, vfold_complaints)
collect_metrics(resample_RF_fit)
...
final_model_fit <- final_fit(final_fit, data = train_data)
```
...
```{r spars_dtm}
inspect(complaints_test_corpus)

# complaints_dtm <- removeSparseTerms(complaints_corpus_dtm2, 0.90)
complaints_test_dtm <- removeSparseTerms(complaints_test_corpus, 0.95)
# complaints_dtm <- removeSparseTerms(complaints_corpus_dtm2, 0.97)
