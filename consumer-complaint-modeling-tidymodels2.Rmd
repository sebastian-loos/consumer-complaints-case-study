...
pre_proc <-
  ...
  # Compute the initial features. This removes the `complaint_raw` column
  ...
  step_stem(complaint) %>%
  # step_text_normalization(-complaint) %>%
...
complaints_model <- 
  ...
  parsnip::decision_tree(cost_complexity = tune(), tree_depth = tune()) |>
  #set engine (rpart)
